...
final class Cosmosfarm_Members {
	...
	public function pre_register_data($fields){
		...
		// 같은 display_name 값이 있는지 체크
		if(isset($_POST['display_name'])){
			
			$user = $wpdb->get_row("SELECT `ID` FROM `{$wpdb->users}` WHERE `display_name`='{$fields['display_name']}'");
			if($user->ID){
				$fields = wpmem_fields();
				$wpmem_themsg = sprintf(__('%s already exists.', 'cosmosfarm-members'), $fields['display_name']['label']);
			}
		}
	}
	...
	public function pre_update_data($fields){
		...
		if(isset($_POST['display_name'])){
			...
			if($fields['display_name'] != $current_user->display_name){
