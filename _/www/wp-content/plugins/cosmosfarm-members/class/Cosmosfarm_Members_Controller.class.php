		...
		if(is_object($api = $this->get_social_api($channel))){
			...
			if($profile->id){
				...
				if(!isset($user->ID) || !$user->ID){
					...
					add_user_meta($user_id, 'cosmosfarm_members_social_id', $social_id);
					add_user_meta($user_id, 'cosmosfarm_members_social_channel', $channel);

					wp_update_user(array('ID'=>$user_id, 'user_url'=>$profile->url, 'user_email'=>$update_email, 'display_name'=>$profile->nickname, 'nickname'=>$profile->nickname));
